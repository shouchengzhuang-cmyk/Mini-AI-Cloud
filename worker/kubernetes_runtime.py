...
class KubernetesRuntime:
    ...
    def _build_pod_template(
        ...
        *,
        ...
        if profile is not None:
            if spec.kubernetes_node_name is None or not spec.kubernetes_node_name.strip():
                raise KubernetesGpuUnavailable(
                    "accelerator Kubernetes tasks require an admitted Kubernetes node"
                )
            validate_kubernetes_dns_subdomain(
                spec.kubernetes_node_name,
                field_name="admitted Kubernetes node",
            )
        ...
        return client.V1PodTemplateSpec(
            ...
            spec=client.V1PodSpec(
                ...
                node_name=(
                    spec.kubernetes_node_name
                    if profile is not None
                    else (self.node_name if spec.mounts else None)
                ),
                node_selector=(
    ...
    def _validate_job_baseline(self, job: object) -> None:
        ...
        if self.app_env == "production" and has_host_path:
            raise KubernetesRuntimeError(
                "refusing to adopt production Kubernetes Job with hostPath"
            )
        node_name = getattr(pod_spec, "node_name", None)
        if self.app_env == "production" and node_name is not None:
            annotations = getattr(getattr(job, "metadata", None), "annotations", None) or {}
            if (
                not isinstance(node_name, str)
                or not node_name.strip()
                or not annotations.get(ACCELERATOR_RESOURCE_ANNOTATION)
                or annotations.get(ALLOCATION_AUTHORITY_ANNOTATION)
                != AllocationAuthority.KUBERNETES_DEVICE_PLUGIN.value
            ):
                raise KubernetesRuntimeError(
                    "refusing to adopt production Kubernetes Job with an unbound nodeName"
                )

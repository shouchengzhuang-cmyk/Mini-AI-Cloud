...
def test_short_and_legacy_console_scripts_resolve_to_the_same_cli() -> None:
    pyproject = Path(__file__).parents[2] / "pyproject.toml"
    metadata = tomllib.loads(pyproject.read_text(encoding="utf-8"))

    scripts = metadata["project"]["scripts"]
    assert scripts["mini-cloud"] == "cli.__main__:app"
    assert scripts["mini-docker-cloud"] == scripts["mini-cloud"]
